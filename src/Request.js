...
class Request {
    constructor(host) {
        this.host = host || "192.168.0.4"
        this.req_id = 0
        this.auth = null
    }
    jsonrpc(method, params = [], needAuth = true) {
    ...
    fetch(jsonrpc) {
        ...
        return post(url, jsonrpc, { headers: header })
            .then(function (res) {
            ...
            .catch(function (err) {
                ...
                if (!err.code && !err.response)
                    return error(-1, 'Unknown error', 'Unknown error on fetch data', '', '')
